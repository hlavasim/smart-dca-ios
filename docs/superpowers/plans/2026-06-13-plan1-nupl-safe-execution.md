...
/// Idempotentní (insert .replace). Vrací počet uložených řádků (0 = skip/neúspěch).
...
```
> Přidej `var lastNuplSyncDay: Int64?` do `UserPreferences` (UserDefaults-backed, jako ostatní preference). NUPL data zůstávají v `nupl_values` i bez nové sync → záloha proti rate-limitu i výpadku.
...
```swift
        let syncNuplUseCase = SyncNuplUseCase(nuplDao: database.nuplDao, marketDataService: marketDataService, userPreferences: userPreferences)
```
